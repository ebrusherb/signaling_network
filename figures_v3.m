...
colormap(mycolormap);
triimage(worstaccuracy,transformed,cornerlabels,mainlabel,'on',interestingi,trilw,{'b','c','d','e','f','g'},'k',.5,1)
...
colormap(mycolormap);
triimage_double(vals,transformed,cornerlabels,'Accuracy','on',interestingi,doubletrilw,{'c','d','e','f','g','h'},'k',.5,1)
...
%         case 4
            ...
            v=get(xlab,'Position');
            set(xlab,'Position',[v(1)+.3 -.58 v(3)])      
    end
    box on
    switch ismember(I,[1 2])
        ...
        case 0
            ylim=get(gca,'Ylim');
            [x,y]=data2norm(0,ylim(2));
            annotation('textbox',[x-.05 .51 .05 .04],'String',['(' alphabet(I+2) ')'],'FitBoxToText','on','FontSize',textfontsz,'FontName',fontname,'EdgeColor','none','VerticalAlignment','middle','HorizontalAlignment','left')
    end
end
...
exstofind=[layers{1}(1) layers{5}(1) layers{7}(1)];
colormap(mycolormap);
triimage(skewvals(numfun,:),transformed,cornerlabels,mainlabel,'on',exstofind,trilw,{'b','c','d'},'k')
