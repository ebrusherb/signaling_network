...
for(i in 1:timesteps+1){
	...
	if(j==4){ #animal 1 signals
		X[2,i]=X[2,i]+bs1
		}
	if(j==5){ #animal 2 signals
		X[1,i]=X[1,i]+bs2
		}	
...
hist(L[[2]][[k]],col=rgb(1,0,0,alph=.25),freq=FALSE,xlim=range(c(L[[2]][[k]],Lb[[2]][[k]])),main=paste("Animal 2's Estimate at t=",bins[k]),xlab="Estimate",breaks=20)
hist(Lb[[2]][[k]],col=rgb(0,0,1,alph=.25),freq=FALSE,add=T,breaks=20)

fight_diff<-seq(0,5,by=1)
signal_feedback<-seq(0,50,by=1)
