function triimage_discrete2(scaledvals,vals,coords,cornerlabels,labels,fourcols,highlight,tohighlight,lw,pointlabels)
...
if strcmp(highlight,'on') 
    for i=1:length(tohighlight)
        p=coords(tohighlight(i),:);
        plot([p(1),p(1)+cstep,p(1)+cstep,p(1),p(1)-cstep,p(1)-cstep,p(1)],[p(2)-2/sqrt(3)*cstep,p(2)-1/sqrt(3)*cstep,p(2)+1/sqrt(3)*cstep,p(2)+2/sqrt(3)*cstep,p(2)+1/sqrt(3)*cstep,p(2)-1/sqrt(3)*cstep,p(2)-2/sqrt(3)*cstep],'Color','k','LineWidth',lw);
