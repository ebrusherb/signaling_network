...
%% coordinate set up
load /Users/eleanorbrush/Dropbox/signaling_network/variables.mat
...
save(filename,'nasheq_ind','nashaccuracy','skewvals','mutvals','accuracy','time','avgthresh','bottomaccuracy','topaccuracy');
%% load data
load /Users/eleanorbrush/Dropbox/signaling_network/triangle_heatmap_output.mat
...
triimage(scaledworst,worstaccuracy,transformed,cornerlabels,mainlabel,'on',interestingi,2,{'a','b','c','d','e','f'},yellow)
...
filename=strcat('/Users/eleanorbrush/Desktop/','pairwiseaccuracy_heatmap','.pdf');
...
for I=1:toplot
    ...
    subplot_tight(2,toplot/2,indvec(I),marg)
...
%     title(titlelab,'FontSize',textfontsz);
    switch I
        case 1
            ...
            legend boxoff
            set(leg,'Position',[.13 .675 .1 .1],'FontName',fontname,'FontSize',labfontsz)
            legendshrink(.5,'best',leg)
        case 4
            xlabel('c','FontSize',textfontsz)
            v=get(get(gca,'xlabel'),'Position');
            set(get(gca,'xlabel'),'Position',[v(1) ypos v(3)])
    end
    box on
    ylim=get(gca,'Ylim');
    [x,y]=data2norm(.6,ylim(2));
    annotation('textbox',[x-.09 y+.08 .05 .04],'String',['(' alphabet(I) ')'],'FitBoxToText','on','FontSize',textfontsz,'FontName',fontname,'EdgeColor','none','VerticalAlignment','middle','HorizontalAlignment','left')
end
...
triimage(scaledbottomacc,bottomaccuracy,transformed,cornerlabels,mainlabel,'on',interestingi,2,{'a','b','c','d','e','f'},yellow)
...
%% bottom accuracy and average group accuracy triangle heatmap figure
...
scaledmat=[scaledbottomacc;scaledaccuracy];
valmat=[bottomaccuracy;accuracy];
triimage_double(scaledmat,valmat,transformed,cornerlabels,'Accuracy','on',interestingi,2,{'a','b','c','d','e','f'},yellow)
...
%             xlabel('Probability stronger animal wins','FontSize',textfontsz)
    ...
    annotation('textbox',[x-.05 .95 .05 .04],'String',['(' alphabet(I) ')'],'FitBoxToText','on','FontSize',textfontsz,'FontName',fontname,'EdgeColor','none','VerticalAlignment','middle','HorizontalAlignment','left')
end
...
%             set(get(gca,'ylabel'),'Position',[xpos v(2) v(3)])
        case 3
            xlab=xlabel('True order of ability','FontSize',labfontsz,'FontName',fontname);
            v=get(xlab,'Position');
            set(xlab,'Position',v+[-10 -3.5 0])
%         case toplot
%             c=colorbar;
%             v=get(c,'position');
%             set(c,'position',[.943 v(2)+.015 v(3) v(4)-.01],'FontSize',labfontsz);
%             yl=get(c,'YLabel');
%             v=get(yl,'Position');
%             set(yl,'String','Time','FontSize',labfontsz,'Rotation',270,'Position',v+[2.4 0 0],'FontName',fontname)
    end
...
funtypes={'# of signals','entropy of signals','# of signalers',' centrality'};
