testdmat=find(extendeddomvals==.5,1,'first')*ones(N);
testdmat(1:N+1:end)=0;
Nd=size(twomat,2);
Nt=size(twomat,3);
l=1;
N=size(testdmat,1);
...
perf=zeros(2,Nd,Nt,Nt); %individual, leak, dominance, thresholds
    ...
    Tvals=zeros(N,testopt_its);
%     Tvals(:,1)=2*ones(N,1);
    Tvals(:,1)=randi([1 Nt],N,1);
    perfvals=zeros(N,testopt_its);

    for i=1:N
        opp_thresh=Tvals([1:(i-1),(i+1):N],1);
        ds=testdmat(i,[1:(i-1),(i+1):N]);
        perfsum=0;
                for q=1:(N-1)
                    if ds(q)<=Nd-1
