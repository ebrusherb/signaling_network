...
for ccount=1:2
    ...
    c2=cvals(ccount,2);
    c3=cvals(ccount,3);
%     [probmat,timemat,accmat]=thresh2mat(seqoptthresh(c,ccount,:),twomat,reshape(dmatssaved(c,:,:),N,N),l);
%     perfvec=c1*(1-accmat)+c2*timemat+c3*(1-probmat);
%     perfvec=sum(perfvec2,2);
    perfvec=thresh2perf([c1 c2 c3],seqoptthresh(c,ccount,:),twomat,reshape(dmatssaved(c,:,:),N,N),l);
    [reshape(perfvalssaved(c,ccount,:),[],1), perfvec reshape(perfvalssaved(c,ccount,:),[],1)-reshape(perfvec,[],1)]
end
